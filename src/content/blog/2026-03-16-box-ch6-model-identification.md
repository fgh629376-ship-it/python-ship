...
- ACF 在 lag $q$ 后截断，PACF 拖尾（指数衰减） → **MA(q)**
- ACF 拖尾（指数/正弦衰减），PACF 在 lag $p$ 后截断 → **AR(p)**
- ACF 和 PACF 两者都拖尾 → **ARMA(p,q)**
...
**序列 A（化学浓度）**：原始 ACF 缓慢衰减；差分后 $\nabla z$: $r_1 \approx -0.4$ 后截断；识别为 IMA(0,1,1)，$\hat{\theta}_1 \approx 0.5$

**序列 B（IBM 股价）**：原始 ACF 缓慢衰减；差分后 $\nabla z$: $r_1 \approx 0.09$ 后截断；识别为 IMA(0,1,1)，$\hat{\theta}_1 \approx -0.1$

**序列 C（温度）**：原始 ACF 缓慢衰减；差分后 $\nabla z$: PACF lag 1 截断；识别为 ARI(1,1,0)，$\hat{\phi}_1 \approx 0.8$

**序列 D（粘度）**：原始 ACF 缓慢衰减；差分后 $\nabla z$: $r_1 \approx -0.05$ 后截断；识别为 IMA(0,1,1)，$\hat{\theta}_1 \approx 0.1$

**序列 E（太阳黑子）**：原始 ACF 伪周期衰减；无需差分；识别为 AR(2)，$\hat{\phi}_1 \approx 1.32, \hat{\phi}_2 \approx -0.63$

**序列 F（批量产率）**：原始 ACF 在 lag 1 截断；无需差分；识别为 MA(1)，$\hat{\theta}_1 \approx 0.5$
