...
**太陽放射 GHI**（$1640$ $\text{kWh/m}^2$/年 ＠上海）

1. **① 変換損失**：GHI → POA（パネル面照射量）— 水平面 → 傾斜面、幾何学的増減
2. **② 入射角（AOI）損失**：2〜4% — 斜め入射時のガラス反射率増大
3. **③ スペクトル損失**：0.5〜2% — 大気質量の変動によるスペクトルシフト
4. **④ 汚損損失（Soiling）**：2〜5% — 埃・鳥の糞・積雪
5. **⑤ 遮蔽損失（Shading）**：0〜10% — 建物・樹木・前列モジュール
6. **⑥ 温度損失**：5〜12% — 温度 $1°\text{C}$ 上昇ごとに効率が 0.3〜0.5% 低下
7. **⑦ モジュールミスマッチ**：1〜3% — 同一ロット内のモジュール出力ばらつき
8. **⑧ 直流配線損失**：1〜3% — ケーブル抵抗による損失
9. **⑨ パワコン損失**：2〜5% — DC$\rightarrow$AC 変換効率
10. **⑩ 交流配線損失**：0.5〜1% — 変圧器＋ケーブル〜系統連系点
...
**発電量の簡易推定**：PR ≈ 0.75〜0.85

$$\text{年間発電量} \approx P_{\text{peak}} \times \text{GHI} \times \text{PR} / 1000$$

**設計最適化の優先順位**：① 遮蔽を最小化 ② 温度を制御 ③ 定期的な洗浄 ④ パワコンの適正選定（DC:AC = 1.1〜1.3）
