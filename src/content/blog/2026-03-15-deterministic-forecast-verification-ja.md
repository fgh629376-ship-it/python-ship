...
κ = 観測値 / 晴天モデル出力。GHI上で直接適用すると**skill scoreが過大評価**される（日変化パターンは「無料の」予測可能性）。
...
同時分布f(x,y)は検証に必要な**すべての情報**を含む。

### 3.1 二つの分解

**Calibration-Refinement分解**: f(x,y) = f(y|x) × f(x)
- f(y|x) → **校正性**: E(Y|X=x) = x が完全校正
- f(x) → **分解能**: 予測値の多様性が高いほど良い

**Likelihood-Base Rate分解**: f(x,y) = f(x|y) × f(y)
...
**COF分解**：
```
MSE = V(Y) + Type 1条件バイアス（最小化）- 分解能（最大化）
```

**COX分解**：
```
MSE = V(X) + Type 2条件バイアス（最小化）- 識別力（最大化）
```
