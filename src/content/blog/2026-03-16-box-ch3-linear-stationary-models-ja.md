...
| 条件 | 要件 | 同値な表述 |
|------|------|-----------|
| **定常** | $\sum|\psi_j| < \infty$ | $\psi(B)$ が $|B| \leq 1$ で収束 |
| **可逆** | $\sum|\pi_j| < \infty$ | $\pi(B)$ が $|B| \leq 1$ で収束 |
...
| 性質 | AR(p) | MA(q) |
|------|-------|-------|
| ACF | 無限に続く（指数減衰／減衰正弦） | **ラグ $q$ 後に打ち切り** |
| PACF | **ラグ $p$ 後に打ち切り** | 無限に続く（指数減衰／減衰正弦） |
| 定常条件 | $\phi(B) = 0$ の根が円外 | 常に定常 |
| 可逆条件 | 常に可逆 | $\theta(B) = 0$ の根が円外 |
| スペクトル | $p(f) \propto 1/|\phi(e^{-i2\pi f})|^2$ | $p(f) \propto |\theta(e^{-i2\pi f})|^2$ |
