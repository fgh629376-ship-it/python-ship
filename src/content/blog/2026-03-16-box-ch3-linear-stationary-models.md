...
| 条件 | 要求 | 等价表述 |
|------|------|----------|
| **平稳** | $\sum|\psi_j| < \infty$ | $\psi(B)$ 在 $|B| \leq 1$ 收敛 |
| **可逆** | $\sum|\pi_j| < \infty$ | $\pi(B)$ 在 $|B| \leq 1$ 收敛 |
...
| 性质 | AR(p) | MA(q) |
|------|-------|-------|
| ACF | 无穷延伸（指数衰减/阻尼正弦） | **lag $q$ 后截断** |
| PACF | **lag $p$ 后截断** | 无穷延伸（指数衰减/阻尼正弦） |
| 平稳条件 | $\phi(B) = 0$ 根在圆外 | 恒平稳 |
| 可逆条件 | 恒可逆 | $\theta(B) = 0$ 根在圆外 |
| 谱 | $p(f) \propto 1/|\phi(e^{-i2\pi f})|^2$ | $p(f) \propto |\theta(e^{-i2\pi f})|^2$ |
